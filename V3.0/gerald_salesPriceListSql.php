	...
	if(isset($_GET['sqlType']))
	{
		if($_GET['sqlType']=='multiplePriceSql')
		{
			...
			foreach($priceIdArray as $key => $priceId)
			{
				...
				if($priceBracket=='-0')		$priceBracket = '-';
				if($priceBracket=='0-')		$priceBracket = '-';
				...
				if($priceId > 0)
				{
					$sql = "
						INSERT INTO `sales_pricelisthistory`
								(	`priceId`, `arkPartId`, `currency`, `accessoryPrice`, `materialPrice`, `subconPrice`, `productionPrice`,`deliveryPrice`, `price`, `priceBracket`, `priceDate`,`quotationDate`, `updateInCharge`)
			...
			header('location:raymond_salesPriceList.php?partId='.$partId);
		}    
		else if($_GET['sqlType']=='priceSql')
		{
			...
			$myfile = $_FILES['quotationPdf']['tmp_name'];
			...
			if($priceId > 0)
			{
				...
				if($queryUpdate)
				{
					$destination = __DIR__ ."/Quotation Files/".$priceId.".pdf";
		...
		else if($_GET['sqlType']=='addPartsSql')
		{
			...
			$sql = "SELECT partId, partNumber, partName FROM cadcam_parts WHERE partNumber LIKE '".trim($partNumber)."' AND revisionId LIKE '".trim($revisionId)."' and customerId=".$customerId." LIMIT 1";
			$queryParts = $db->query($sql);
			if($queryParts AND $queryParts->num_rows == 0)
			{
				$resultQueryParts = $queryParts->fetch_assoc();

				$sql = "INSERT INTO	cadcam_parts 
							(	partNumber,					partName,		itemGroup,		revisionId,					customerId,		customerPartNumber, 	status)
					VALUES	(	'".trim($partNumber)."',	'".trim($partName)."',	'".trim($itemGroup)."',	'".trim($revisionId)."',	".$customerId.", '".trim($partNumber)."',	2) ";
				$queryInsert = $db->query($sql);
			}
			...
			header('location:raymond_salesPriceList.php?partId='.$maxPartId);
		}
	}	
?>
