<?php
	...
	$count = $queryPosition;
	$sql = "SELECT partId, customerPartNumber, partNumber, partName, revisionId, partNote, customerId, itemGroup FROM cadcam_parts ".$sqlFilter." ORDER BY partId DESC ".$sqlLimit;
	$sqlMain = $sql;
	$query = $db->query($sql);
	if($query->num_rows > 0)
	{
		...
		while($result = $query->fetch_array())
		{
			...
			if($_SESSION['idNumber'] == "")
			{
				...
				//{
					$tableContent .= "
					<tr>
						<td align = 'center'>".++$count."<br>".$priceFlag."</td>
